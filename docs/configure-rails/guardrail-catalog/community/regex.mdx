...
Tool rails check one tool call's arguments, or one tool result's content, against patterns scoped to that specific tool. Configure them under `rails.tool_output.per_tool` (calls) and `rails.tool_input.per_tool` (results), each keyed by tool name, alongside the matching patterns under `rails.config.regex_detection.tool_output`/`tool_input`:
...
`$argument=query` scopes the output check to the call's `query` field only, so other arguments (for example a request ID) never reach the regex, avoiding a false-positive match on unrelated data. Omit `$argument=` to check the call's full argument set. It applies to tool output (call) surfaces only; tool input (result) surfaces have no argument structure to scope and do not accept it.

For the full per-tool surface reference, refer to [Rail Engine Support](/reference/rail-engine-support#tool-output-surfaces).
...
`regex check tool output`/`regex check tool input` are `IORails`-only capabilities by design, scoped per tool under `rails.tool_output.per_tool`/`rails.tool_input.per_tool`.
