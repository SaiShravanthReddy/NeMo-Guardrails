...
| `--metrics-exporter` | Expose the IORails non-streaming admission-queue metrics (`guardrails.nonstream.*`) on a scrape endpoint. Supported values: `prometheus` and `none`. Use `none` to disable export. Overrides `NEMO_GUARDRAILS_SERVER_METRICS_EXPORTER`. Default: `none`. |
