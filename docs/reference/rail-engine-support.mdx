...
Tool output surfaces run against one tool call at a time, configured under `rails.tool_output.per_tool` and keyed by tool name.
...
| Flow | Rail | LLMRails | IORails | Notes |
|---|---|:---:|:---:|---|
| `regex check tool output` | Regex Detection | ✗ | ✓ | IORails-only capability by design |

### Tool Input Surfaces

Tool input surfaces run against one tool result at a time, configured under `rails.tool_input.per_tool` and keyed by tool name.
Per-tool checks are an `IORails`-only capability by design:

| Flow | Rail | LLMRails | IORails | Notes |
|---|---|:---:|:---:|---|
| `regex check tool input` | Regex Detection | ✗ | ✓ | IORails-only capability by design |

For a worked per-tool configuration example, including the `$argument=` scoping option, refer to [Regular Expression Detection Integration](/configure-guardrails/guardrail-catalog/community/regex).
