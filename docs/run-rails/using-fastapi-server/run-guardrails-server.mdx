...
Set `metrics.enabled: true` in the guardrails configuration, then start the server with the IORails engine and Prometheus exporter enabled.
...
Then scrape the endpoint to check that the queue gauges appear:
